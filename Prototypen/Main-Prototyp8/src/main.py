...
class CheckoutItem(QGraphicsObject):
    # Statische Variablen für ALLE Bilder (Normal & SB)
    ...
    def __init__(
        ...
        # 1. Bilder laden (nur einmalig beim ersten Start)
    ...
    def boundingRect(self):
        # Nutzt die globalen Konstanten für die Größe
        return QRectF(0, 0, CHECKOUT_WIDTH, CHECKOUT_HEIGHT)

    def paint(self, painter: QPainter, option, widget=None):
        # 2. Das richtige Bild auswählen
        pixmap = None

        if self.c_type == "SB":
            # SB Kasse
            if self.orientation == "Left":
                pixmap = CheckoutItem._pixmap_sb_left
            else:
                pixmap = CheckoutItem._pixmap_sb_right
        else:
            # Normale Kasse
            if self.orientation == "Left":
                pixmap = CheckoutItem._pixmap_normal_left
            else:
                pixmap = CheckoutItem._pixmap_normal_right

        # 3. Zeichnen
        if pixmap and not pixmap.isNull():
            painter.drawPixmap(self.boundingRect().toRect(), pixmap)
        else:
            # FALLBACK (falls Bild fehlt): Farbiges Rechteck
            color = (
                QColor("#607D8B")
            ...
            # Orientierungsstreifen (Fallback)
...
class CustomerItem(QGraphicsEllipseItem):
    ...
    def process_scanning(self):
        if self.items_scanned < self.inventory:
            ...
            self.state = "LEAVING"
            # WICHTIG: NICHT assigned_checkout_id hier löschen!
        self.update()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Prototyp 16: Queue Logic Final (Projekt: Piep)")
    ...
    def handle_scene_click(self, pos):
        if self.is_drawing_mode:
            ...
            if len(self.current_route_points) > 1:
                ...
                for x in self.current_route_points[1:]:
                    p.lineTo(x)
