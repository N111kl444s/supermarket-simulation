...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Prototyp 19: Map System (Projekt: Piep)")
    ...
    def update_paths(self):
        map_dir = MAPS_DIR / self.current_map_name
        ...
        self.routes_file = map_dir / "routes.json"
        self.shelves_file = map_dir / "shelves.json"
        self.checkouts_file = map_dir / "checkouts.json"
        self.waiting_area_file = map_dir / "waiting_area.json"
    ...
    def create_top_left_quadrant(self):
        ...
        self.map_selector.currentTextChanged.connect(self.change_map)
        map_layout.addWidget(self.map_selector, 1)
        btn_new_map = QPushButton("+")
        btn_new_map.setFixedWidth(30)
        btn_new_map.clicked.connect(self.create_new_map)
        map_layout.addWidget(btn_new_map)
        l.addLayout(map_layout)
    ...
    def refresh_map_list(self):
        ...
        if MAPS_DIR.exists():
            for d in MAPS_DIR.iterdir():
                if d.is_dir():
                    self.map_selector.addItem(d.name)
        self.map_selector.setCurrentText(self.current_map_name)
        self.map_selector.blockSignals(False)

    def create_new_map(self):
        text, ok = QInputDialog.getText(self, "Neue Map", "Name der Map:")
        if ok and text:
            new_path = MAPS_DIR / text
            if new_path.exists():
                QMessageBox.warning(self, "Fehler", "Map existiert bereits!")
            else:
                try:
                    new_path.mkdir()
                    self.refresh_map_list()
                    self.map_selector.setCurrentText(text)
                except:
                    pass
    ...
    def toggle_delete_mode(self, checked):
        self.is_deleting_objects = checked
        if checked:
            self.delete_objects_button.setStyleSheet(
