...
COLOR_QUEUE_HIGHLIGHT = QColor("#FF00FF")  # Magenta für Highlight
    ...
    def tick(self):
        if self.state == "FINISHED_SHOPPING":
            ...
            (
                ...
                else self.new_wait_target()
            )
            return
        if self.state == "WAITING_AT_SHELF":
            self.wait_ticks -= 1
            (self.set_state_return() if self.wait_ticks <= 0 else None)
            return
        self.move_towards_target()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Prototyp 12: Queue Highlight (Projekt: Piep)")
        ...
        self.highlight_queues = False  # NEU: Flag für Queue-Highlight
    ...
    def draw_debug_elements(self):
        ...
        # Nur zeigen wenn toggled ODER placement mode ODER delete mode
        ...
        if self.settings["show_checkouts"]:
            for cd in self.checkouts_data:
                ...
                if should_draw_queue:
                    # Wenn Highlight an ist (Settings Dialog offen), dann dickes Magenta, sonst normales Grau
