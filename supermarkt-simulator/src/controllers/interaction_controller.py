...
Updated: Added missing QGraphicsView import.
"""

# FIX: QGraphicsView hier hinzugefügt
...
class InteractionController(QObject): 
    ...
    def set_tool(self, tool_name, params=None, button_ref=None):
        ...
        if button_ref and button_ref.isChecked():
            ...
            if self.view.sim_view: 
                # Das hier hat den Fehler geworfen, weil QGraphicsView fehlte
    ...
    def edit_object_position(self, obj_spec):
        ...
        def update_ang(na):
            ...
            self.visual_controller.draw_map_elements(self.map_manager, selected_spec=obj_spec)
        def update_var(nv):
            if obj_type == "shelf": self.map_manager.all_shelves[obj_spec["index"]]["variant"] = nv
            self.visual_controller.draw_map_elements(self.map_manager, selected_spec=obj_spec)
    ...
    def handle_checkout_click(self, checkout_id):
        ...
        if current_mode == "Editor": self.edit_object_position({"type": "checkout", "id": checkout_id})
    ...
    def handle_shelf_click(self, index):
        current_mode = self.view.mode_combo.currentText()
        if current_mode == "Editor": self.edit_object_position({"type": "shelf", "index": index})
