...
class InteractionController:
    """
    Verwaltet Tools und Klicks auf der Scene.
    """
    ...
    def __init__(
        ...
        self.view = view
        self.sim_manager = sim_manager  # FIX: Referenz für is_running Check
    ...
    def handle_scene_click(self, pos):
        ...
        if self.active_tool in ["route", "start_route", "exit_route"]:
            ...
            self.map_manager.all_shelves.append(pos)
            self.visual_controller.draw_map_elements(self.map_manager)
    ...
    def handle_area_created(self, rect):
        if self.active_tool == "waiting_area":
            self.map_manager.waiting_area_rect = rect
        elif self.active_tool == "start_area":
            self.map_manager.start_area_rect = rect
        elif self.active_tool == "exit_area":
            self.map_manager.exit_area_rect = rect
        self.visual_controller.draw_map_elements(self.map_manager)
    ...
    def edit_object_position(self, obj_spec):
        ...
        if obj_type == "shelf":
            ...
            if idx < len(self.map_manager.all_shelves):
                pt = self.map_manager.all_shelves[idx]
                cx, cy, name = pt.x(), pt.y(), f"Regal #{idx+1}"
            ...
            if c:
                ...
                cur_ori, cur_ang = c.get("orientation", "Right"), c.get(
                    "angle", 0
                )

        dlg = ObjectPositionDialog(
            name, cx, cy, orientation=cur_ori, angle=cur_ang, parent=self.view
        )

        def update_pos(nx, ny):
            if obj_type == "shelf":
                self.map_manager.all_shelves[obj_spec["index"]] = QPointF(
                    nx, ny
                )
            elif obj_type == "checkout":
                c = next(
    ...
    def edit_checkout_config(self, checkout_id):
        # FIX: Verwende mode_combo.currentText() statt current_mode_text
        current_mode = self.view.mode_combo.currentText()

        # Logik: Im Editor immer erlaubt. Im Simulations-Modus nur erlaubt, wenn NICHT läuft.
        if current_mode == "Simulation":
            if self.sim_manager.is_running:
                return  # Blockieren während Simulation läuft
