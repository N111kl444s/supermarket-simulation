...
Refactored: 
- Prevents selecting objects while using tools (drawing routes).
"""
...
class InteractionController(QObject): 
    ...
    def _init_temp_path(self, color):
        ...
        self.current_route_path_item.setParentItem(self.visual_controller.map_group)
    ...
    def handle_scene_click(self, global_pos):
        ...
        if self.active_tool in ["route", "start_route", "exit_route"]:
            ...
            dot = QGraphicsEllipseItem(local_pos.x()-4, local_pos.y()-4, 8, 8)
            ...
            if ok:
                new_shelf = {
                    ...
                    "angle": self.current_angle, "variant": variant, "mirrored": self.current_mirrored
                }
                ...
                self.visual_controller.draw_map_elements(self.map_manager, selected_spec=spec)
            ...
            new_checkout = {
                "id": new_id, "x": local_pos.x(), "y": local_pos.y(),
                "type": self.active_tool_params.get("type"), "orientation": self.active_tool_params.get("ori"),
                "open": True, "skill": "Azubi", "max_queue": 5, "angle": self.current_angle 
            }
    ...
    def edit_object_position(self, obj_spec):
        ...
        if obj_type == "shelf":
            ...
            cx, cy = data["x"], data["y"]
            cur_ang = data.get("angle", 0); cur_var = data.get("variant", 1)
            name = f"Regal #{idx+1}"
    ...
    def handle_checkout_click(self, checkout_id):
        # FIX: Wenn ein Zeichen-Tool aktiv ist, Klick ignorieren (nicht selektieren)
