...
Visual Controller.
Refactored:
- Queue Logic: UNIFIED Direction. 
  Both Left and Right orientations now use 'angle' directly.
  This aligns Right checkouts with the correct behavior observed on Left checkouts.
"""

import math
from PyQt6.QtGui import QColor, QPen, QBrush, QPixmap, QPainterPath
from PyQt6.QtWidgets import QGraphicsPixmapItem, QGraphicsPathItem, QGraphicsEllipseItem, QGraphicsRectItem
...
class VisualController:
    ...
    def draw_map_elements(self, map_manager, selected_spec=None, highlight_queues=False):
        ...
        for idx, s_data in enumerate(map_manager.all_shelves):
            sx = s_data.get("x", 0); sy = s_data.get("y", 0)
            show = self.settings["show_shelves"] or is_selected("shelf", idx)
            if show:
                s = ShelfItem(sx, sy, index=idx, size=self.settings.get("size_shelf", 32), show_label=show_s_nums, angle=s_data.get("angle", 0), variant=s_data.get("variant", 0), mirrored=s_data.get("mirrored", False))
        ...
        if self.settings["show_routes"]: self._draw_routes(map_manager)
        self.scene.blockSignals(False)

    def _draw_single_checkout(self, cd, is_selected, show_id=True):
        ori = cd.get("orientation", "Right"); angle = cd.get("angle", 0); c_type = cd["type"]
        ...
        cw = self.settings.get("size_checkout_width", 100); ch = self.settings.get("size_checkout_height", 100)
        
        ls = self.settings.get("size_checkout_light", 8)
        
        ci = CheckoutItem(cd["x"], cd["y"], c_type, ori, cd["open"], False, cd.get("id"), lo, width=cw, height=ch, angle=angle, show_id=show_id, light_size=ls)
        ...
        if self.settings["show_cashiers"]:
            ...
            center_x = cx + cw / 2; center_y = cy + ch / 2
            lox, loy = lo
            if lox == 0 and loy == 0: lox, loy = cw / 2, 10
            p_unrot_x = cx + lox; p_unrot_y = cy + loy
            rad = math.radians(angle)
            tx = p_unrot_x - center_x; ty = p_unrot_y - center_y
            rx = tx * math.cos(rad) - ty * math.sin(rad); ry = tx * math.sin(rad) + ty * math.cos(rad)
            light_x = rx + center_x; light_y = ry + center_y
            light_item = QGraphicsEllipseItem(light_x - ls/2, light_y - ls/2, ls, ls)
            col = Qt.GlobalColor.green if cd.get("open", True) else Qt.GlobalColor.red
            light_item.setBrush(QBrush(col)); light_item.setPen(QPen(Qt.GlobalColor.black, 1))
            light_item.setZValue(50); light_item.setParentItem(self.map_group)
    ...
    def _draw_queue_visuals(self, cd):
        cw = self.settings.get("size_checkout_width", 100); ch = self.settings.get("size_checkout_height", 100)
        spacing = self.settings.get("dist_queue_spacing", 20); dot_size = self.settings.get("size_queue_dot", 4)
        max_q = cd.get("max_queue", 5); cx, cy = cd["x"], cd["y"]; angle = cd.get("angle", 0)
        ...
        center_x = cx + cw / 2; center_y = cy + ch / 2
        p_unrot_x = cx + off[0]; p_unrot_y = cy + off[1]
        rad = math.radians(angle)
        tx = p_unrot_x - center_x; ty = p_unrot_y - center_y
        rx = tx * math.cos(rad) - ty * math.sin(rad); ry = tx * math.sin(rad) + ty * math.cos(rad)
        start_point_x = rx + center_x; start_point_y = ry + center_y
        
        # FIX: UNIFIED Direction Logic
        # Both Left and Right should behave like the Left one (Angle)
        dir_rad = math.radians(angle)
            
        dir_x = math.cos(dir_rad); dir_y = math.sin(dir_rad)
        
        for i in range(max_q):
            px = start_point_x + dir_x * (i * spacing); py = start_point_y + dir_y * (i * spacing)
            dot = QGraphicsEllipseItem(px - dot_size/2, py - dot_size/2, dot_size, dot_size)
            dot.setBrush(QBrush(QColor(255, 165, 0, 180))); dot.setPen(QPen(Qt.GlobalColor.white, 1))
            dot.setZValue(20); dot.setParentItem(self.map_group); self.queue_debug_items.append(dot)

    def _draw_routes(self, map_manager):
        def draw(routes, col):
            for pts in routes.values():
                if len(pts) > 1:
                    pp = QPainterPath(); pp.moveTo(pts[0]); [pp.lineTo(p) for p in pts[1:]]
                    pi = QGraphicsPathItem(pp)
                    pen = QPen(col, 2, Qt.PenStyle.DotLine); pen.setCosmetic(True); pi.setPen(pen)
                    pi.setZValue(4); pi.setParentItem(self.map_group); self.route_debug_items.append(pi)
        draw(map_manager.shop_routes, QColor(100,100,100,100)); draw(map_manager.start_routes, COLOR_BLUE); draw(map_manager.exit_routes, COLOR_RED)

    def sync_customers(self, models):
        current_set = set(models); to_remove = []
        for model, item in self.customer_items.items():
            if model not in current_set:
                item.setParentItem(None); 
                if item.scene(): self.scene.removeItem(item)
                to_remove.append(model)
        for m in to_remove: del self.customer_items[m]
        for model in models:
            if model not in self.customer_items:
                item = CustomerItem(model, size=self.settings.get("size_customer", 32))
                item.setParentItem(self.map_group); self.customer_items[model] = item
            else: self.customer_items[model].sync_visuals()

    def _clear_dynamic_items(self):
        for i in self.shelf_items + self.checkout_items + self.cashier_items + self.route_debug_items + self.queue_debug_items + self.light_items:
        ...
        self.shelf_items.clear(); self.checkout_items.clear(); self.cashier_items.clear(); self.route_debug_items.clear(); self.queue_debug_items.clear(); self.light_items.clear()