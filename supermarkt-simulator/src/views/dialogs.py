...
class OffsetDialog(QDialog):
    ...
    def add_xy_row(self, layout, prefix, title):
        for suffix in ["_left", "_right"]:
            ...
            layout.addRow(f"{title} ({'Links' if 'left' in suffix else 'Rechts'}) X/Y:", self.create_hbox(sb_x, sb_y))
    ...
    def emit_live_update(self):
        ...
        for key, widgets in self.inputs.items():
            if key == "customer_path_offset": continue
...
class CheckoutConfigDialog(QDialog):
    def __init__(self, data, parent=None):
        ...
        current_staff = data.get("cashier_skill") or data.get("skill") or "Azubi"
        ...
        layout.addWidget(buttons)
    def get_data(self):
        return {"max_queue": self.sb_queue.value(), "open": self.cb_open.isChecked(), "cashier_skill": self.combo_staff.currentText()}
