...
Refactored:
- NEW: Info Box Layout with Payment Icon (Stack layout).
- NEW: Paying State Animation (Big Icon).
- Loads Cash/Card icons.
"""
...
class CustomerItem(QGraphicsPixmapItem):
    ...
    def _load_images(cls):
        ...
        for name in IMG_CUSTOMERS_NORMAL:
            p = IMAGE_DIR / name
            if p.exists(): cls._pixmaps_normal.append(QPixmap(str(p)))

        for name in IMG_CUSTOMERS_DISABLED:
            p = IMAGE_DIR / name
            if p.exists(): cls._pixmaps_disabled.append(QPixmap(str(p)))

        for name in IMG_CUSTOMERS_HANDHELD:
            p = IMAGE_DIR / name
            if p.exists(): cls._pixmaps_handheld.append(QPixmap(str(p)))

        for name in IMG_CUSTOMERS_HANDHELD_DISABLED:
            p = IMAGE_DIR / name
            if p.exists(): cls._pixmaps_handheld_disabled.append(QPixmap(str(p)))
    ...
    def _load_icons(cls):
        ...
        if icon_path.exists(): cls._scanner_icon = QPixmap(str(icon_path))
        else: cls._scanner_icon = QPixmap()
            
        # Cash
        cash_path = ASSETS_DIR / "icons" / "cash.png"
        if cash_path.exists(): cls._cash_icon = QPixmap(str(cash_path))
        else: cls._cash_icon = QPixmap()
            
        # Card
        card_path = ASSETS_DIR / "icons" / "card.png"
        if card_path.exists(): cls._card_icon = QPixmap(str(card_path))
        else: cls._card_icon = QPixmap()
    ...
    def _create_info_box(self):
        ...
        if self.model.payment_method == "cash" and self._cash_icon and not self._cash_icon.isNull():
            self.icon_payment.setPixmap(self._cash_icon)
        elif self.model.payment_method == "card" and self._card_icon and not self._card_icon.isNull():
    ...
    def set_visuals(self):
        ...
        if self.model.uses_handheld:
            if self.model.is_disabled: pool = (self._pixmaps_handheld_disabled or self._pixmaps_disabled)
            else: pool = self._pixmaps_handheld or self._pixmaps_normal
        elif self.model.is_disabled: pool = self._pixmaps_disabled
        else: pool = self._pixmaps_normal

        if not pool: pool = self._pixmaps_normal
        if not pool: return
    ...
    def sync_visuals(self):
        ...
        # --- LAYOUT LOGIC ---
        padding_x = 3; padding_y = 2; spacing_y = 1
        
        count_str = str(current_count)
        if self.text_item.text() != count_str: self.text_item.setText(count_str)
        txt_rect = self.text_item.boundingRect()
        txt_w, txt_h = txt_rect.width(), txt_rect.height()
        
        has_handheld = (self.model.uses_handheld and not self.icon_handheld.pixmap().isNull())
        hh_w, hh_h = 0, 0
        if has_handheld:
            target_hh = txt_h * 0.8
        ...
        if is_paying:
            ...
            self.icon_handheld.setVisible(False)
            ...
            pay_h = target_big
            content_w = pay_w; content_h = pay_h
            ...
            content_h = txt_h
            if has_payment: content_h += spacing_y + pay_h
            if has_handheld: content_h += spacing_y + hh_h
        ...
        if self.pulse_val > 0:
            self.pulse_val -= 0.1
            if self.pulse_val < 0: self.pulse_val = 0
